...
class lumise_admin extends lumise_lib{
	...
	protected function process_save_data($field, $data) {
		...
		if ($field['type'] == 'stages') {
			...
			$stages = $this->dejson($data[$field['name']]);
			
			foreach(array('front', 'back', 'left', 'right') as $s) {
				
				$name = "product-upload-".$s;
				
				if (isset($stages->{$s}) && $_POST['old-'.$name] != $stages->{$s}->url && $stages->{$s}->source == 'uploads') {
					
					$_field = array("name" => $name, "path" => 'products'.DS);
					
					$_data = array();
					
					if (isset($_POST[$name]) && !empty($_POST[$name]))
						$_data[$name] = $this->dejson($_POST[$name]);
					else $_data[$name] = '';
						
					$_data['errors'] = array();
						
					$processed = $this->process_upload($_field, $_data);
					
					if (count($processed['errors']) === 0) {
						$stages->{$s}->url = $processed[$name];
					}else $data['errors'] = array_merge($data['errors'], $processed['errors']);
				
				}
				
				if (
					((isset($stages->{$s}) && $stages->{$s}->source == 'raws') || !isset($stages->{$s})) && 
					$_POST['old-'.$name.'-source'] == 'uploads' &&
			...
			$data[$field['name']] = $this->enjson($stages);
				...
				if ($check !== 1) {
					...
					if (isset($process['error'])) {
						...
						if (isset($process['thumbn']) && isset($field['thumbn'])) {
							$data[$field['thumbn']] = $lumise->cfg->upload_url.str_replace(TS, '/', $path.$process['thumbn']);
						}
			...
			foreach ($tags as $field) {
				...
				if (is_array($post_tags) && count($post_tags) > 0) {
					foreach ($post_tags as $tag) {
						...
						$tid = $lumise->db->rawQuery("SELECT `id` FROM `{$lumise->db->prefix}tags` WHERE `slug`='{$this->slugify($tag)}'");
	...
	public function process_callback($id = 0, $cb = '') {
		...
		switch ($cb) {
			case 'edit-cms-product':
				$data = $lumise->db->rawQuery("SELECT `name`,`color`,`stages` FROM `{$lumise->db->prefix}products` WHERE `id`=$id");
	...
	public function process_settings_data($args) {
		...
		foreach ($args['tabs'] as $tab => $tab_fields) {
			foreach ($tab_fields as $i => $field) {
				if (isset($field['name']) && array_key_exists($field['name'], $lumise->cfg->settings)) {
					$args['tabs'][$tab][$i]['value'] = $lumise->cfg->settings[$field['name']];
					if (isset($_POST['lumise-section']) && $_POST['lumise-section'] == 'settings')
						$data = $this->process_save_data($field, $data);
				}
			}
		}
		...
		if (isset($_POST['lumise-section']) && $_POST['lumise-section'] == 'settings') {
			...
			if (count($data['errors']) === 0) {
				...
				foreach ($data as $key => $val) {
					...
					if ($key == 'primary_color' && $lumise->get_option($key) != $val) {
						$this->render_primary_colors($val);
					}
					
					if ($key == 'custom_css' && $lumise->get_option($key) != $val) {
						$this->render_custom_css($val);
					}
				...
				$lumise->apply_filter('after_save_settings', $data);
	...
	public function render_primary_colors($color) {
		
		global $lumise;
		
		$primary = $lumise->cfg->root_path.'assets'.DS.'css'.DS.'primary_color.css';
		$path = $lumise->cfg->root_path.'assets'.DS.'css'.DS.'colors.css';
		...
		if (isset($do_action)) {
			switch ($do_action) {
				...
				case 'verify-license' : 
					...
					$check = $this->main->lib->lumise_conn(
				...
				case 'do-update':
					...
					if ($key === null || empty($key) || strlen($key) != 36 || count(explode('-', $key)) != 5) {
						...
						$context = $this->main->lib->lumise_conn(null, array("Download: yes", "Key: ".$key));
...
class lumise_helper {
	...
	public function breadcrumb($lumise_page, $type = null) {
		...
		$arr = array(
			'cliparts' => array(
				...
				'link'   => $lumise_router->getURI().'lumise-page=cliparts',
				'child' => array(
					'clipart' => array(
						'type'   => '',
						'title'  => $lumise->lang('Clipart'),
						'link'   => $lumise_router->getURI().'lumise-page=clipart',
					),
					'categories' => array(
						'type'   => 'cliparts',
						'title'  => $lumise->lang('Categories'),
						'link'   => $lumise_router->getURI().'lumise-page=categories&type=cliparts',
					),
					'category' => array(
						'type'   => 'cliparts',
						'title'  => $lumise->lang('Category'),
						'link'   => $lumise_router->getURI().'lumise-page=category&type=cliparts',
					),
					'tags' => array(
						'type'   => 'cliparts',
						'title'  => $lumise->lang('Tags'),
						'link'   => $lumise_router->getURI().'lumise-page=tags&type=cliparts',
					),
					'tag' => array(
						'type'   => 'cliparts',
						'title'  => $lumise->lang('Tag'),
						'link'   => $lumise_router->getURI().'lumise-page=tag&type=cliparts',
					),
				),
			),
			'designs' => array(
				'title' => $lumise->lang('Designs'),
				'link'   => $lumise_router->getURI().'lumise-page=designs',
				'child' => array(
					'design' => array(
						'type'   => '',
						'title'  => $lumise->lang('Design'),
						'link'   => $lumise_router->getURI().'lumise-page=design',
					),
				),
			),
			'templates' => array(
				'title' => $lumise->lang('Templates'),
				'link'   => $lumise_router->getURI().'lumise-page=templates',
				'child' => array(
					'template' => array(
						'type'   => '',
						'title'  => $lumise->lang('Template'),
						'link'   => $lumise_router->getURI().'lumise-page=template',
					),
					'categories' => array(
						'type'   => 'templates',
						'title'  => $lumise->lang('Categories'),
						'link'   => $lumise_router->getURI().'lumise-page=categories&type=templates',
					),
					'category' => array(
						'type'   => 'templates',
						'title'  => $lumise->lang('Category'),
						'link'   => $lumise_router->getURI().'lumise-page=category&type=templates',
					),
					'tags' => array(
						'type'   => 'templates',
						'title'  => $lumise->lang('Tags'),
						'link'   => $lumise_router->getURI().'lumise-page=tags&type=templates',
					),
					'tag' => array(
						'type'   => 'templates',
						'title'  => $lumise->lang('Tag'),
						'link'   => $lumise_router->getURI().'lumise-page=tag&type=templates',
					),
				),
			),
			'products' => array(
				'title' => $lumise->lang('Products'),
				'link'   => $lumise_router->getURI().'lumise-page=products',
				'child' => array(
					'product' => array(
						'type'   => '',
						'title'  => $lumise->lang('Product'),
						'link'   => $lumise_router->getURI().'lumise-page=product',
					),
					'categories' => array(
						'type'   => 'products',
						'title'  => $lumise->lang('Product Categories'),
						'link'   => $lumise_router->getURI().'lumise-page=categories&type=products',
					),
					'category' => array(
						'type'   => 'products',
						'title'  => $lumise->lang('Add New Category'),
						'link'   => $lumise_router->getURI().'lumise-page=category&type=products',
					),
				),
			),
			'shapes' => array(
				'title' => $lumise->lang('Shapes'),
				'link'   => $lumise_router->getURI().'lumise-page=shapes',
				'child' => array(
					'shape' => array(
						'type'   => '',
						'title'  => $lumise->lang('Shape'),
						'link'   => $lumise_router->getURI().'lumise-page=shape',
					),
				),
			),
			'printings' => array(
				'title' => $lumise->lang('Printing Type'),
				'link'   => $lumise_router->getURI().'lumise-page=printings',
				'child' => array(
					'printing' => array(
						'type'   => '',
						'title'  => $lumise->lang('Printing'),
						'link'   => $lumise_router->getURI().'lumise-page=printing',
					),
				),
			),
			'fonts' => array(
				'title' => $lumise->lang('Fonts'),
				'link'   => $lumise_router->getURI().'lumise-page=fonts',
				'child' => array(
					'font' => array(
						'type'   => '',
						'title'  => $lumise->lang('Edit Font'),
						'link'   => $lumise_router->getURI().'lumise-page=font',
					)
				),
			),
			'languages' => array(
				'title' => $lumise->lang('Languages'),
				'link'   => $lumise_router->getURI().'lumise-page=languages',
				'child' => array(
					'language' => array(
						'type'   => '',
						'title'  => $lumise->lang('Language'),
						'link'   => $lumise_router->getURI().'lumise-page=language',
					),
				),
			),
			'orders' => array(
				'title' => $lumise->lang('Orders'),
				'link'   => $lumise_router->getURI().'lumise-page=orders',
			),
			'settings' => array(
				'title' => $lumise->lang('Settings'),
				'link'   => $lumise_router->getURI().'lumise-page=settings',
			),
		);
		...
		foreach ($arr as $keys => $values) {
			...
			if ($keys == $lumise_page) {
				...
				$html .= '<li><a href="'.$lumise_router->getURI().'lumise-page=dashboard">'.$lumise->lang('Dashboard').'</a></li><li><span>'.$values['title'].'</span></li>';
			...
			if (isset($values['child'])) {
				...
				if (isset($values['child'][$lumise_page]) && $values['child'][$lumise_page]['type'] == $type) {
					...
					$html .= '<li><a href="'.$lumise_router->getURI().'lumise-page=dashboard">'.$lumise->lang('Dashboard').'</a></li><li><a href="'.$values['link'].'">'.$values['title'].'</a></li>';
	...
	public function import_sample_shapes($shapes) {
		...
		$lumise->redirect($lumise_router->getURI().'lumise-page=shapes');
