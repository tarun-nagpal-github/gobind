...
class lumise_router {
	...
	protected $_lumise_pagess;
	protected $_lumise_pages;
	protected $_action;
	...
	public function __construct($lumise_page = '', $load_assets = true) {
		...
		$this->menus = array(
			'dashboard' => array(
				...
				'child'	=> array(
					...
					'updates' => array(
						'title' => $lumise->lang('Updates').(!empty($check_update) && isset($check_update->version) && $check_update->version > LUMISE ? ' <span class="update-notice">1</span>' : ''),
		...
		if (isset($this->_lumise_pages[$this->_lumise_page]) && $this->_lumise_pages[$this->_lumise_page])
			$this->_allow = true;
	...
	public function update_notice() {
		...
		if(
			...
			$this->check_update->version > LUMISE
		) {
	...
	public function display() {
		...
		if ($this->_allow) {
			$this->update_notice();
			include $this->_admin_path . 'pages' .DS. $this->_lumise_page . '.php';
		} else {
