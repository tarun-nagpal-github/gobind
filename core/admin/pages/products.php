...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise_router->getURI();?>lumise-page=product" class="add_new"><?php echo $lumise->lang('Add New Product Base'); ?></a>
			<?php
								...
								foreach ($products['rows'] as $value) { ?>
									...
									<tr>
										...
										<td><?php
											...
											$color = explode(':', isset($value['color'])?$value['color']:'');

											if(isset($value['stages']) && !empty($value['stages'])) {
												$stages = $lumise_admin->dejson($value['stages']);
												if ($stages !== null) {
													echo '<ul>';
													foreach ($stages as $stage) {
														echo '<li><img style="background:'.$color[0].'" src="'.(
																(isset($stage->source) && $stage->source == 'raws') ? 
																$lumise->cfg->assets_url.'raws/' : 
																$lumise->cfg->upload_url
															).$stage->url.'" height="100" /></li>';
													}
													echo '</ul>';
												}
											}
