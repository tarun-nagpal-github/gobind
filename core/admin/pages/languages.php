<?php
	...
	if (isset($_POST['change_language']) && !empty($_POST['change_language'])) {
    ...
    $config = array(
		...
		'total_page'    => $languages['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise_router->getURI().'lumise-page=languages&tpage={page}',
	    'link_first'    => $lumise_router->getURI().'lumise-page=languages',
	);
	...
	<div class="lumise_content">
		...
		<div class="lumise_message noti">
			<em class="lumise_suc">
				...
				<a href="<?php echo $lumise_router->getURI();?>lumise-page=settings">
		...
		<div class="lumise_option">
				<div class="left">
					<form action="<?php echo $lumise_router->getURI();?>lumise-page=languages" method="post">
									...
									<?php
										...
										foreach ($langs as $lang) {
											echo '<option value="'.$lang.'"'.(
												(isset($_SESSION[$prefix.'lang']) && $_SESSION[$prefix.'lang'] == $lang) ? ' selected' : ''
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise_router->getURI().'lumise-page=languages" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
...
(function($){
	...
	$('#lumise-scan-language').on('click', function(){
		...
		if (!code || code === '') {
			return alert('<?php echo $lumise->lang('Please select a language before scanning'); ?>');
		}
	...
	$('a#lumise-auto-translate').on('click', function(e){
		...
		$(this).after('<span style="color: #aaa;font-weight:400;font-style: italic"><i class="fa fa-spinner fa-spin fa-fw"></i> <?php echo $lumise->lang('Translating'); ?> <span id="lumise-auto-translating">0</span> of '+list.length+'</span>').remove();

		var do_translate = function() {

			if (curent >= list.length)
			 return $('#lumise-auto-translating').parent().html('<?php echo $lumise->lang('Translate complete!'); ?>');

			if (list.eq(curent).length === 0)
				return;

			var el = list.eq(curent++),
				id = el.data('id'),
				origin = $('#lumise-lang-original-'+id).html(),
				text = $('#lumise-lang-text-'+id).html();

			$('#lumise-auto-translating').html(curent);
			...
			$.get('//translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl='+code+'&dt=t&q='+encodeURI(text), function(res){

				if (res && res[0] && res[0][0] && res[0][0][0] && res[0][0][0] != text) {
					...
					$.ajax({
						...
						success: function(re){
							$('#lumise-lang-text-'+re.id).html(re.text).after(' <i class="fa fa-check"></i>');
						}
					});
				}else $('#lumise-lang-text-'+code).after(' <i class="fa fa-check"></i>');
