<?php
    ...
    $config = array(
		...
		'total_page'    => $printings['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise_router->getURI().'lumise-page=printings&tpage={page}',
	    'link_first'    => $lumise_router->getURI().'lumise-page=printings',
	);
...
<div class="lumise_wrapper">
	...
	<div class="lumise_content">
		...
		<div class="lumise_header">
			...
			<a href="<?php echo $lumise_router->getURI();?>lumise-page=printing" class="add_new"><?php echo $lumise->lang('Add New Printing'); ?></a>
			<?php
		...
		<div class="lumise_option">
			<div class="left">
				<form action="<?php echo $lumise_router->getURI();?>lumise-page=printings" method="post">
							...
							if ( is_array($printings['rows']) && count($printings['rows']) > 0 ) {
								...
								foreach ($printings['rows'] as $value) { ?>
									...
									<tr>
										...
										<td>
											<a href="<?php echo $lumise_router->getURI();?>lumise-page=printing&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php if(isset($value['title'])) echo $value['title']; ?></a>
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$lumise_router->getURI().'lumise-page=printings" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$lumise->lang('Back To Lists').'</a>';
					}
					else
