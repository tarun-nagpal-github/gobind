    ...
    class lumise_connector {
        ...
        public function __construct() {
            ...
            $this->config = array(
                ...
                "upload_path" => 'D:\\xampp\\htdocs\\lumise\\lumise\\data\\',
    			"upload_url" => 'http://localhost/lumise/lumise/data/',
        ...
        public function url($f = 0) {
			...
			return ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') || $_SERVER['SERVER_PORT'] == 443) ? 'https' : 'http'."://".$_SERVER['HTTP_HOST'].$uri;
    	...
    	public function process_reset() {
    		...
    		if (!lumise_secure::check_nonce('RESET-SECURITY', $nonce)) {
    			...
    			if ($limit[2] >= 5 && time()-$limit[3] < 60*60) {
    				...
    				$token = rand(45734455645856384, 2457344425566435859673874);
    	...
    	public function add_to_cart($data){
    		$this->redirect($this->config['editor_url'].'cart.php');
    	}
        ...
        public function update() {
			...
			if (!empty($connector_content)) {
				...
				$connector_content = str_replace(array(
					'D:\\xampp\\htdocs\\lumise\\lumise\\data\\',
					'http://localhost/lumise/lumise/data/',
					'localhost',
	                'root',
	                '',
	                'lumise',
